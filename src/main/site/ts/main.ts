...
const suggestionsUL: HTMLUListElement | null = document.querySelector("ul#suggestions");
const selectSun: HTMLSelectElement | null  = document.querySelector("select#sun");
const selectMoon: HTMLSelectElement | null = document.querySelector("select#moon");
const selectRising: HTMLSelectElement | null = document.querySelector("select#rising");

// Here, when the value of sun is changed, we will call the method postAndUpdate.
// TODO: Do the same for moon and rising
if (selectSun && selectMoon && selectRising) {
  selectSun.addEventListener("change", postAndUpdate);
  selectMoon.addEventListener("change", postAndUpdate);
  selectRising.addEventListener("change", postAndUpdate);
}
...
function postAndUpdate(): void {
  ...
  const postParameters: MatchesRequestData = {
    "sun": selectSun!.value,
    "moon": selectMoon!.value,
    "rising": selectRising!.value
...
function updateSuggestions(matches: string[]): void {
  ...
  for (const match of matches) {
    suggestionsUL!.innerHTML += `<li tabindex="0">${match}</li>`
  }
}
...
document.addEventListener("keyup", (e) => {
  if (e.key == "r") {
    selectSun!.value = "Taurus";
    selectMoon!.value = "Leo";
    selectRising!.value = "Sagittarius";
  }
})
...
async function updateValues(sunval: string, moonval: string, risingval: string): Promise<void>{
  ...
  selectSun!.value = sunval;
  selectMoon!.value = moonval;
  selectRising!.value = risingval;
}
